...
std::vector<Point> Astar::PathPlanning(Point _startPoint, Point _targetPoint)
{
    // Get variables
...
vector<Point>Astar::GetPath_sep(Node* TailNode){
    PathList.clear();
    ...
    for(int i = 0;i < length;i++)
    {
        path1.push_back(PathList.back()->point);
        PathList.pop_back();
    }
